...
class MonoDETR(nn.Module):
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        """
        Feature map sizes from backbone:
        features[0]: [batch_size, 512, 48, 160]
        features[1]: [batch_size, 1024, 24, 80]
        features[2]: [batch_size, 2048, 12, 40]

        pos[0]: [batch_size, 256, 48, 160]
        pos[1]: [batch_size, 256, 24, 80]
        pos[2]: [batch_size, 256, 12, 40]
        """

        # * shape of src[0]: torch.Size([16, 256, 48, 160])
        # * shape of src[1]: torch.Size([16, 256, 24, 80])
        # * shape of src[2]: torch.Size([16, 256, 12, 40])
        # * shape of src[3]: torch.Size([16, 256, 6, 20])
