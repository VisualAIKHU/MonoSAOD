...
def build_lr_scheduler(cfg, optimizer, last_epoch):
    if cfg['type'] == 'cos':
        def lr_lbmd(cur_epoch):
            max_epoch = cfg.get('max_epoch', 195)
            min_lr = float(cfg.get('min_lr', 1e-6))  # Convert to float
            min_lr_ratio = min_lr / optimizer.defaults['lr']
            return min_lr_ratio + (1 - min_lr_ratio) * 0.5 * (1 + math.cos(math.pi * cur_epoch / max_epoch))
    elif cfg['type'] == 'step':
        def lr_lbmd(cur_epoch):
            cur_decay = 1
            ...
            return max(cur_decay, 0.01)
        
    lr_scheduler = lr_sched.LambdaLR(optimizer, lr_lbmd, last_epoch=last_epoch)
    warmup_lr_scheduler = None
    if cfg['warmup']:
        warmup_lr_scheduler = CosineWarmupLR(optimizer, num_epoch=10, init_lr=0.00001)
