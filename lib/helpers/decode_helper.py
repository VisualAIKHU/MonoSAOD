...
#             # cls_id (class_id), alpha(heading angle), bbox(2D bbox 4),
#             # dimensions(w,h,l 3), locations(x,y,z 3), ry(yaw angle), score(confidence score * depth score)
#             preds.append([cls_id, alpha] + bbox + dimensions.tolist() + locations.tolist() + [ry, score])
#         results[info['img_id'][i]] = preds
#     return results


def decode_detections(dets, info, calibs, cls_mean_size, threshold, return_uncertainty=False):
    '''
    NOTE: THIS IS A NUMPY FUNCTION
    input: dets, numpy array, shape in [batch x max_dets x dim]
    input: img_info, dict, necessary information of input images
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            if return_uncertainty:
                preds.append([cls_id, alpha] + bbox + dimensions.tolist() + locations.tolist() + [ry, score, dets[i, j, -1]])
            else:
                preds.append([cls_id, alpha] + bbox + dimensions.tolist() + locations.tolist() + [ry, score])
        results[info['img_id'][i]] = preds
    return results
...
def extract_dets_from_outputs(outputs, K=50, topk=50):
    ...
    sigma = outputs['pred_depth'][:, :, 1: 2] # this is log(σ²), the model's raw log-variance
    sigma = torch.exp(-sigma) # exp(-log σ²) = 1/σ²
