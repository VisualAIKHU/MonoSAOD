...
from lib.helpers.trainer_helper_stage1 import Trainer
from lib.datasets.kitti.kitti_dataset_stage1 import KITTI_Dataset
...
def main():
    ...
    # Promote Stage 1's best checkpoint to ckpt/stage1_best.pth for Stage 2 to consume.
    best_src = Path(output_path) / 'checkpoint_best.pth'
    ckpt_dst = Path('ckpt') / 'stage1_best.pth'
    ckpt_dst.parent.mkdir(parents=True, exist_ok=True)
    if best_src.is_file():
        shutil.copy2(best_src, ckpt_dst)
        logger.info('[stage1] promoted {} -> {}'.format(best_src, ckpt_dst))
    else:
        logger.warning('[stage1] expected best checkpoint at {} not found; ckpt/ not updated'.format(best_src))
